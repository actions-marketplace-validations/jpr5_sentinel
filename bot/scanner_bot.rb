...
module Bot
    class ScannerBot
        ...
        def build_advisory_issue_body(repo:, advisory_findings:)
            ...
            body += "### Protect this repo permanently\n\n"
            body += "This scan found vulnerabilities that existed in your workflows today. "
            body += "New ones get introduced with every workflow change. "
            body += "Add Sentinel to your CI to catch them automatically on every PR:\n\n"
            body += "```yaml\n"
            body += "# .github/workflows/sentinel.yml\n"
            body += "name: Security Scan\n"
            body += "on:\n"
            body += "  pull_request:\n"
            body += "    paths: ['.github/workflows/**']\n"
            body += "  push:\n"
            body += "    branches: [main]\n"
            body += "    paths: ['.github/workflows/**']\n"
            body += "\n"
            ...
            body += "    timeout-minutes: 10\n"
            body += "    steps:\n"
            body += "      - uses: actions/checkout@v4\n"
            body += "        with:\n"
            body += "          persist-credentials: false\n"
            body += "      - uses: jpr5/sentinel@v1\n"
            body += "        with:\n"
            body += "          severity: high\n"
            body += "```\n\n"
            body += "Free, open-source, no secrets or write permissions needed. "
            body += "[Learn more](https://sentinel.copilotkit.dev)\n\n"
